...
const reSize = async (image, ukur1 = 100, ukur2 = 100) => {
	return new Promise(async(resolve, reject) => {
		try {
			...
			const result = await read.resize({ w: ukur1, h: ukur2 }).getBuffer('image/jpeg')
			resolve(result)
		} catch (e) {
			reject(e)
		}
...
const generateProfilePicture = async (buffer) => {
	...
	return {
			img: await cropped.scaleToFit(720, 720).getBuffer('image/jpeg'),
			preview: await cropped.scaleToFit(720, 720).getBuffer('image/jpeg')
	}
}
